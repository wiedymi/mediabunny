...
import { readBytes, readU32Le } from '../reader.js';
import { textDecoder, binarySearchLessOrEqual, binarySearchExact } from '../misc.js';
import { parseMainHeader, parseStreamHeader, parseBitmapInfoHeader, parseWaveFormatEx, parseIndexEntry, aviVideoFourccToCodec, aviAudioFormatTagToCodec, parseStreamChunkId, AVIIF_KEYFRAME, } from './avi-misc.js';
// Base class following Matroska pattern
class AviTrackBacking {
    constructor(demuxer, streamInfo) {
        this.demuxer = demuxer;
        this.streamInfo = streamInfo;
        this.packetToIndex = new WeakMap();
    }
    getId() {
    ...
    async getFirstTimestamp() {
        if (this.streamInfo.packets.length === 0)
            return 0;
    ...
    async getFirstPacket(options) {
        ...
        if (options.metadataOnly) {
            const packet = new EncodedPacket(new Uint8Array(), 'key', packetInfo.timestamp, packetInfo.duration, firstKeyIndex, packetInfo.entry.size);
    ...
    async getPacket(timestamp, options) {
        ...
        const index = binarySearchLessOrEqual(this.streamInfo.presentationTimestamps, timestamp, x => x.timestamp);
        if (index === -1) {
            ...
            if (options.metadataOnly) {
                const packet = new EncodedPacket(new Uint8Array(), (packetInfo.entry.flags & AVIIF_KEYFRAME) ? 'key' : 'delta', packetInfo.timestamp, packetInfo.duration, packetIndex, packetInfo.entry.size);
    ...
    async getNextPacket(packet, options) {
        ...
        if (nextIndex >= this.streamInfo.packets.length)
            return null;
        const packetInfo = this.streamInfo.packets[nextIndex];
        if (options.metadataOnly) {
            const newPacket = new EncodedPacket(new Uint8Array(), (packetInfo.entry.flags & AVIIF_KEYFRAME) ? 'key' : 'delta', packetInfo.timestamp, packetInfo.duration, nextIndex, packetInfo.entry.size);
    ...
    async getKeyPacket(timestamp, options) {
        ...
        if (options.metadataOnly) {
            const packet = new EncodedPacket(new Uint8Array(), 'key', packetInfo.timestamp, packetInfo.duration, bestKeyIndex, packetInfo.entry.size);
    ...
    async getNextKeyPacket(packet, options) {
        ...
        const keyIndex = binarySearchExact(this.streamInfo.keyPacketIndices, index, x => x);
        let nextKeyIndex = -1;
        if (keyIndex !== -1) {
        ...
        if (options.metadataOnly) {
            const newPacket = new EncodedPacket(new Uint8Array(), 'key', packetInfo.timestamp, packetInfo.duration, nextKeyIndex, packetInfo.entry.size);
...
class AviVideoTrackBacking extends AviTrackBacking {
    constructor() {
        super(...arguments);
...
class AviAudioTrackBacking extends AviTrackBacking {
    constructor() {
        super(...arguments);
...
export class AVIDemuxer extends Demuxer {
    ...
    async readChunkHeader(position) {
        ...
        if (!slice || slice.remainingLength < 8)
            return null;
    ...
    async readPacket(entry, timestamp, duration = 0, sequenceNumber = -1) {
        ...
        if (!data)
            return null;
        return new EncodedPacket(data, (entry.flags & AVIIF_KEYFRAME) ? 'key' : 'delta', timestamp, duration, sequenceNumber, data.byteLength);
    }
